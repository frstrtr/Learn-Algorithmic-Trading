...
for close_price in close:
    ...
    if ema_macd == 0:
        ...
        ema_macd = (macd - ema_macd) * K_slow + \
            ema_macd  # signal is EMA of MACD values

    macd_values.append(macd)
    macd_signal_values.append(ema_macd)
    macd_historgram_values.append(macd - ema_macd)

nicehash_data = nicehash_data.assign(
    ClosePrice=pd.Series(close, index=nicehash_data.index))
nicehash_data = nicehash_data.assign(FastExponential10DayMovingAverage=pd.Series(
    ema_fast_values, index=nicehash_data.index))
nicehash_data = nicehash_data.assign(SlowExponential40DayMovingAverage=pd.Series(
    ema_slow_values, index=nicehash_data.index))
nicehash_data = nicehash_data.assign(MovingAverageConvergenceDivergence=pd.Series(
    macd_values, index=nicehash_data.index))
nicehash_data = nicehash_data.assign(Exponential20DayMovingAverageOfMACD=pd.Series(
    macd_signal_values, index=nicehash_data.index))
nicehash_data = nicehash_data.assign(MACDHistorgram=pd.Series(
    macd_historgram_values, index=nicehash_data.index))

close_price = nicehash_data['ClosePrice']
ema_f = nicehash_data['FastExponential10DayMovingAverage']
ema_s = nicehash_data['SlowExponential40DayMovingAverage']
macd = nicehash_data['MovingAverageConvergenceDivergence']
ema_macd = nicehash_data['Exponential20DayMovingAverageOfMACD']
macd_histogram = nicehash_data['MACDHistorgram']


fig = plt.figure()
ax1 = fig.add_subplot(311, ylabel='ETH price in BTC')
...
macd_histogram.plot(ax=ax3, color='r', kind='bar',
                    legend=True, use_index=False)
plt.show()
